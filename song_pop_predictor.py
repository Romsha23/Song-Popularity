...
def train_dataset():
    ...
    models = {
        ...
        'XGBoost': xgb.XGBRegressor(objective='reg:squarederror', n_estimators=500, learning_rate=0.01, random_state=42)
    }
    ...
    for name, model in models.items():
        ...
        rmse = np.sqrt(mse)
        st.success(f"{name} trained in {time.time() - start:.2f} sec, RMSE: {rmse:.2f}")
